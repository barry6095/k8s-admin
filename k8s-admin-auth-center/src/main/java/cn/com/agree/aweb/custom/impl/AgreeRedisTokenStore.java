package cn.com.agree.aweb.custom.impl;
...
public class AgreeRedisTokenStore implements TokenStore {
...
//    private static final String UNAME_TO_ACCESS = "uname_to_access:";
	...
	public AgreeRedisTokenStore(RedisConnectionFactory connectionFactory) {
	...
	public void storeAccessToken(OAuth2AccessToken token, OAuth2Authentication authentication) {
		...
		byte[] authToAccessKey = this
				.serializeKey("auth_to_access:" + this.authenticationKeyGenerator.extractKey(authentication));
		...
		try {
			...
			if (refreshToken != null && refreshToken.getValue() != null) {
				...
				byte[] refreshToAccessKey = this
						.serializeKey("refresh_to_access:" + token.getRefreshToken().getValue());
	...
	public void removeAccessToken(String tokenValue) {
		...
		try {
			...
			if (authentication != null) {
				...
				byte[] clientId = this
						.serializeKey("client_id_to_access:" + authentication.getOAuth2Request().getClientId());
				conn.openPipeline();
	...
	public Collection<OAuth2AccessToken> findTokensByClientIdAndUserName(String clientId, String userName) {
		...
		if (byteList != null && byteList.size() != 0) {
			...
			Iterator<byte[]> var7 = byteList.iterator();

			while (var7.hasNext()) {
				byte[] bytes = var7.next();
				OAuth2AccessToken accessToken = this.deserializeAccessToken(bytes);
	...
	public Collection<OAuth2AccessToken> findTokensByClientId(String clientId) {
		...
		if (byteList != null && byteList.size() != 0) {
			List<OAuth2AccessToken> accessTokens = new ArrayList<>(byteList.size());
			Iterator<byte[]> var6 = byteList.iterator();
